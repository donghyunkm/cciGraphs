...
class MultilayerNode2Vec(torch.nn.Module): 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if WITH_PYG_LIB and p == 1.0 and q == 1.0:
            ...
            if p == 1.0 and q == 1.0:
                raise ImportError(
                    f"'{self.__class__.__name__}' " f"requires either the 'pyg-lib' or " f"'torch-cluster' package"
                )
            else:
                raise ImportError(f"'{self.__class__.__name__}' " f"requires the 'torch-cluster' package")
        ...
        self.p = p
        self.q = q
        self.num_negative_samples = num_negative_samples_per_layer * self.n_layers
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}({self.embedding.weight.size(0)}, " f"{self.embedding.weight.size(1)})"