...
### Neuropeptideergic communication as directed multilayer graphs 
...
Graph neural networks combine features of nodes that are adjacent according to the underlying graph [@brody2021attentive].

A nonlinear function ∅ of aggregated (sum here) weighted node features determines updated node representation.

In GATv2, the node features are weighted using a function of node pairs, $\theta$. Functions $\phi$ and $\theta$ are learned to optimize a per-node classification objective.
...
**Spectral clustering** frames the problem as an eigen decomposition [@von2007tutorial]. Specifically, the eigenvectors of the Laplacian matrix are used as features for clustering.
...
In the joint setting, we sample random walks from all layers to determine the neighborhood for each node (as opposed to neighborhoods constructed from only 1 layer). Note that the peptidergic multilayer graph does not contain any edges between layers.
